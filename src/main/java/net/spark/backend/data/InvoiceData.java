...
public class InvoiceData {
    ...
    public void importData(BasicProfileImp profileImp, Invoice invoice) {
        ...
        profileImp.setSellerName("Aquí Ltd");
        profileImp.setSellerLineOne("Domaine de Labourdonnais");
        profileImp.setSellerLineTwo("");
        profileImp.setSellerPostcode("31803");
        profileImp.setSellerCityName("Mapou");
        profileImp.setSellerCountryID("MU");
        //profileImp.addSellerTaxRegistration(TaxIDTypeCode.FISCAL_NUMBER, "201/113/40209");
        profileImp.addSellerTaxRegistration(TaxIDTypeCode.VAT, "27352151");
        ...
        profileImp.setInvoiceCurrencyCode("MUR");
    }
    ...
    public void importBasicData(BasicProfileImp profileImp, Invoice invoice) {
        ...
        for (Map.Entry<Double, Double> t : taxes.entrySet()) {
            ...
            ttA += (total - tA);
            ltN += tA;
            profileImp.addApplicableTradeTax(format2dec(total - tA), "MUR", TaxTypeCode.VALUE_ADDED_TAX, format2dec(tA), "MUR", format2dec(tax));
        }
        profileImp.setMonetarySummation(format2dec(ltN), "MUR",
            format2dec(0), "MUR",
            format2dec(0), "MUR",
            format2dec(ltN), "MUR",
            format2dec(ttA), "MUR",
    ...
    public void importComfortData(ComfortProfileImp profileImp, Invoice invoice) {
        ...
        profileImp.addPaymentMeans(
                ...
                new String[]{"Bank Details:"},
                "Account Name:", "Aqui Ltd",
                "", "",
                "MU49MCBL0903000443769222000MUR", "", "",
                "", "", "",
                "000443769222", "", "MCB");
 /*       profileImp.addPaymentMeans(
                PaymentMeansCode.PAYMENT_TO_BANK_ACCOUNT,
        ...
        for (Item item : invoice.getItems()) {
            ...
            profileImp.addIncludedSupplyChainTradeLineItem(
                    ...
                    null,
                    format4dec(item.getProduct().getPrice()), "MUR", null, null,
                    null, null, null, null,
                    null, null, null, null,
        ...
        for (Map.Entry<Double, Double> t : taxes.entrySet()) {
            ...
            profileImp.addApplicableTradeTax(
                    format2dec(total - tA), "MUR", TaxTypeCode.VALUE_ADDED_TAX,
                    null, format2dec(tA), "MUR",
