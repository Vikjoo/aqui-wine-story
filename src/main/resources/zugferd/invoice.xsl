...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/rsm:CrossIndustryDocument">
		<html>
			...
			<body>
				<img src="logo.png" alt="Aqui Company - logo" />
				<br />
				<xsl:apply-templates />
			</body>
		</html>
	...
	<xsl:template match="rsm:SpecifiedSupplyChainTradeTransaction">
		<table width="100%" border="0" id="addresses">
			<tr>
				...
				<td valign="Top" width="40%">
					<xsl:for-each
						select="ram:ApplicableSupplyChainTradeAgreement/ram:SellerTradeParty">
						<xsl:call-template name="address" />
						<xsl:text>T:58540796</xsl:text>
						<br/> 
						<xsl:text>BRN:C07003219</xsl:text>
				...
				<td valign="Top" width="40%">
					<xsl:for-each
						select="ram:ApplicableSupplyChainTradeAgreement/ram:BuyerTradeParty">
						<xsl:call-template name="address" />
					</xsl:for-each>
				</td>
			</tr>
			...
			<tbody>
				<xsl:for-each select="ram:IncludedSupplyChainTradeLineItem">
					<tr>
						...
						<td align="Right">
							<xsl:call-template name="twodecimals">
								<xsl:with-param name="number"
									select="ram:SpecifiedSupplyChainTradeAgreement/ram:GrossPriceProductTradePrice/ram:ChargeAmount" />
							</xsl:call-template>
						</td>
						<td align="Right">
							<xsl:call-template name="twodecimals">
								<xsl:with-param name="number"
									select="ram:SpecifiedSupplyChainTradeDelivery/ram:BilledQuantity" />
							</xsl:call-template>
						</td>
						<td align="Right">
							<xsl:value-of
								select="ram:SpecifiedSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:LineTotalAmount" />
						</td>
						<td align="Right">
							<xsl:value-of
								select="ram:SpecifiedSupplyChainTradeSettlement/ram:ApplicableTradeTax/ram:ApplicablePercent" />
							%
						</td>
						<td align="Right">
							<xsl:call-template name="calcTax">
								<xsl:with-param name="basis"
									select="ram:SpecifiedSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:LineTotalAmount" />
								<xsl:with-param name="tax"
									select="ram:SpecifiedSupplyChainTradeSettlement/ram:ApplicableTradeTax/ram:ApplicablePercent" />
							</xsl:call-template>
						</td>
						...
						<td>
							<xsl:value-of
								select="ram:SpecifiedSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:LineTotalAmount/@currencyID" />
						</td>
					</tr>
				</xsl:for-each>
			</tbody>
		...
		<table width="100%" id="totals">
			...
			<tbody>
				<xsl:for-each
					select="ram:ApplicableSupplyChainTradeSettlement/ram:ApplicableTradeTax">
					<tr>
						<td align="Right">
				...
				<tr>
					...
					<td class="total" align="Right">
						<xsl:value-of
							select="ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:TaxBasisTotalAmount" />
					</td>
					<td class="total" align="Right">
						<xsl:value-of
							select="ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:TaxTotalAmount" />
					</td>
					<td class="total" align="Right">
						<xsl:value-of
							select="ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:GrandTotalAmount" />
					</td>
					<td class="total">
						<xsl:value-of
							select="ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementMonetarySummation/ram:GrandTotalAmount/@currencyID" />
					</td>
				</tr>
			</tbody>
		</table>
		<table id="wireinfo">
			<tr>
				<td colspan="4">
					...
					full.
					<!-- <xsl:value-of
						select="ram:ApplicableSupplyChainTradeSettlement/ram:PaymentReference" /> -->
				</td>
			</tr>
			<tr>
				<!-- <th></th> -->
				<th class="wireheader">Bank</th>
				<th class="wireheader">A/c name</th>
				<th class="wireheader">A/c number</th>
				<th class="wireheader">IBAN</th>
			</tr>
			<xsl:for-each
				select="ram:ApplicableSupplyChainTradeSettlement/ram:SpecifiedTradeSettlementPaymentMeans">
				<tr>
					<!-- <td></td> -->
					<td>
						<xsl:value-of
							select="ram:PayeeSpecifiedCreditorFinancialInstitution/ram:Name" />
					</td>
					<td>Aqui Ltd&#032;&#032;&#032;&#032;&#032;&#032;&#032;</td>
					<td><xsl:value-of
	...
	<xsl:template name="calcWithTax">
		...
		<xsl:call-template name="twodecimals">
			<xsl:with-param name="number"
				select="$basis + (($basis * $tax) div 100)" />
		</xsl:call-template>
	</xsl:template>
